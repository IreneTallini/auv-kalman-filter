...
class Filter:
    ...
    def __init__(self, acc_std, range_std, init_pos_ned, n_ranges, 
                 threshold, anch_pos, hz_depth, hz_cvm, beacon_err, debug=0) :
    ...
    def M(self, p, ps):
        ...
        dp         = np.concatenate((p[:,:2] - ps[:,:2], np.ones((ps.shape[0], 1) )), axis=1)
        jacob      = dp/np.linalg.norm(p-ps, axis=1).reshape(dp.shape[0], 1)
        ...
        six_ones   = np.ones((1, 6))
        new_jacob  = np.concatenate(
            ( 
                np.concatenate((first, six_ones), axis=1),
                np.concatenate((three_ones, second, three_ones), axis=1),
                np.concatenate((six_ones, third), axis=1)
            ), axis=0
        )
        return new_jacob
