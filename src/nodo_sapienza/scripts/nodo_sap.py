    ...
    def __init__(self):
        ...
        if filter_type == "all":
            ...
            self.dvl_sub = rospy.Subscriber(
                "team/sensors/dvl", Velocity, self.dvl_callback
            )
        if filter_type == "imu_range":
            self.ekf = k_filter_imu_range.Filter(
                vel_std,
                head_std,
                range_std,
                init_pos_ned,
        ...
        if filter_type == "range":
            self.ekf = k_filter_range.Filter(
                acc_std,
                range_std,
                init_pos_ned,
                num_anchors,
        ...
        if filter_type == "range_beacon_error":
            self.ekf = k_filter_range_beacon_error.Filter(
                acc_std,
                range_std,
                init_pos_ned,
                num_anchors,
        ...
        if filter_type == "all_beacon_error":
            self.ekf = k_filter_all_beacon_error.Filter(
                vel_std,
                head_std,
                range_std,
                init_pos_ned,
        ...
        self.depth_sub = rospy.Subscriber(
            "team/sensors/depth", Float32, self.depth_callback
        )
        self.imu_sub = rospy.Subscriber(
            "team/sensors/imu", AttitudeEuler, self.imu_callback
        )
        self.range_sub = rospy.Subscriber(
            "team_sapienza/range", Range, self.range_callback
        )
...
if __name__ == "__main__":
    ...
    for i in range(num_anchors):
        n_anch, e_anch, _ = pm.geodetic2ned(
            anch_pos[i][0],
            anch_pos[i][1],
            ref_pos[2],
            ref_pos[0],
    ...
    init_pos_ned = pm.geodetic2ned(
        init_pos[0], init_pos[1], ref_pos[2], ref_pos[0], ref_pos[1], ref_pos[2]
    )
