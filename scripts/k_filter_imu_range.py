...
class Filter:
    ...
    def __init__(self, vel_std, yaw_std, range_std, n_ranges, 
                 threshold, anch_pos, hz_imu, hz_depth, debug=0) :

        ############################## IMU and DEPTH #########################################
        self.Q        = np.diag([yaw_std, vel_std])**2     # Error covariance
        self.state    = np.array([ [30],[30], [5] ])       # State
    ...
    def L(self, yaw) :
        ...
        vel   = self.state[2, 0]
        jacob = np.array([[-self.dt*vel*np.sin(yaw), 0], 
    ...
    def F(self, yaw) :
        ...
        return np.array([[1, 0, self.dt*np.cos(yaw)], 
    ...
    def update(self, anch_id, rg, ts):
        ...
        if not_zero >= self.threshold:
            ...
            self.P      = np.eye(3) - np.dot(np.dot(K, H), self.P)
